...
sidebar_label: "One-Line Installer for All Linux Systems"
...
If you are installing on macOS, make sure to check the [install documentation for macOS](/docs/installing/macos) before continuing.


> :bulb: Tip
>
> If you are unsure whether you want nightly or stable releases, read the [installation guide](/docs/installing#nightly-vs-stable-releases).  
...
- `--claim-proxy`
  Specify a proxy to use when connecting to the cloud in the form of `http://[user:pass@]host:ip` for an HTTP(S) proxy. See [connecting through a proxy](/docs/configuring/connect-agent-to-cloud#connect-through-a-proxy) for details.
- `--claim-only`
...
- `--prepare-offline-install-source`
  Instead of insallling the agent, prepare a directory that can be used to install on another system without needing to download anything. See our [offline installation documentation](/docs/installing/offline-systems) for more info.
