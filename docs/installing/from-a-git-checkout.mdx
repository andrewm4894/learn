...
Otherwise, [perform a cleanup](/docs/installing/from-a-git-checkout#perform-a-cleanup-in-your-netdata-repo)
...
-   `--disable-telemetry`: Opt-out of [anonymous statistics](/docs/configuring/securing-netdata-agents/anonymous-telemetry-events) we use to make
    Netdata better.
...
-   `--local-files`: Used for [offline installations](/docs/installing/offline-systems). Pass four file paths: the Netdata
...
Unlike the [`kickstart.sh`](/docs/installing/one-line-installer-for-all-linux-systems), the `netdata-installer.sh` script does
not allow you to automatically [connect](/docs/configuring/connect-agent-to-cloud) your node to Netdata Cloud immediately after installation.

See the [connect to cloud](/docs/configuring/connect-agent-to-cloud) doc for details on connecting a node with a manual installation of Netdata.
