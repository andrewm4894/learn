...
sidebar_label: "Health Configuration Reference"
learn_status: "Published"
learn_rel_path: "Alerting"
sidebar_position: "704000"
---
# Configure alerts
...
You can use health alerts in conjunction with any of Netdata's [collectors](/docs/data-collection) (see
the [supported collector list](/docs/data-collection/monitor-anything)) to monitor the health of your systems, containers, and
...
[infrastructure](/docs/architecture/the-role-of-netdata-cloud), you must configure each node with the same health configuration
files.
...
- The `[health]` section in `netdata.conf`. By editing the daemon's behavior, you can disable health monitoring
    ...
    [daemon configuration](/docs/configuring/daemon-configuration#health-section-options) for a table of
...
Navigate to your [Netdata config directory](/docs/configuring/configuration) and
...
[health management API](/docs/alerting/health-api-calls). The
...
[simple pattern](/docs/miscellaneous/libnetdata/simple-patterns) that
...
[health management API](/docs/alerting/health-api-calls).
...
To write a new health entity into a new file, navigate to your [Netdata config directory](/docs/configuring/configuration),
...
- A few lines use space-separated lists to define how the entity behaves. You can use `*` as a wildcard or prefix with
    `!` for a negative match. Order is important, too! See our [simple patterns docs](/docs/miscellaneous/libnetdata/simple-patterns) for
    more examples.
...
list of [simple patterns](/docs/miscellaneous/libnetdata/simple-patterns). For example,
...
list of [simple patterns](/docs/miscellaneous/libnetdata/simple-patterns). For
...
The value is a space-separated list of [simple patterns](/docs/miscellaneous/libnetdata/simple-patterns). For
...
The full [database query API](/docs/rest-api/queries) is supported. In short:

- `METHOD` is one of  the available [grouping methods](/docs/rest-api/queries#grouping-methods) such as `average`, `min`, `max` etc.
     This is required.
...
- `OPTIONS` is a space separated list of `percentage`, `absolute`, `min2max`, `unaligned`,
     `match-ids`, `match-names`. Check the [badges](/docs/rest-api/netdata-badges) documentation for more info.
...
Defines the list of labels present on a host. See our [host labels guide](/docs/configuring/organize-systems-metrics-and-alerts) for
...
See our [simple patterns docs](/docs/miscellaneous/libnetdata/simple-patterns) for more examples.
...
### Example 8 - [Anomaly rate](/docs/ml-and-troubleshooting/machine-learning-ml-powered-anomaly-detection#anomaly-rate) based CPU dimensions alert

Warning if 5 minute rolling [anomaly rate](/docs/ml-and-troubleshooting/machine-learning-ml-powered-anomaly-detection#anomaly-rate) for any CPU dimension is above 5%, critical if it goes above 20%:
...
### Example 9 - [Anomaly rate](/docs/ml-and-troubleshooting/machine-learning-ml-powered-anomaly-detection#anomaly-rate) based CPU chart alert

Warning if 5 minute rolling [anomaly rate](/docs/ml-and-troubleshooting/machine-learning-ml-powered-anomaly-detection#anomaly-rate) averaged across all CPU dimensions is above 5%, critical if it goes above 20%:
...
### Example 10 - [Anomaly rate](/docs/ml-and-troubleshooting/machine-learning-ml-powered-anomaly-detection#anomaly-rate) based node level alert

Warning if 5 minute rolling [anomaly rate](/docs/ml-and-troubleshooting/machine-learning-ml-powered-anomaly-detection#anomaly-rate) averaged across all ML enabled dimensions is above 5%, critical if it goes above 20%:
...
The `lookup` line will use the `anomaly_rate` dimension of the `anomaly_detection.anomaly_rate` ML chart to calculate the average [node level anomaly rate](/docs/ml-and-troubleshooting/machine-learning-ml-powered-anomaly-detection#node-anomaly-rate) over the last 5 minues.
...
a [Netdata simple pattern](/docs/miscellaneous/libnetdata/simple-patterns) to create
...
Use a [simple pattern](/docs/miscellaneous/libnetdata/simple-patterns)! One example of a simple pattern is a single wildcard
(`*`).
...
our [simple patterns documentation](/docs/miscellaneous/libnetdata/simple-patterns).
...
You can compile Netdata with [debugging](/docs/configuring/netdata-daemon#debugging) and then set in `netdata.conf`:
