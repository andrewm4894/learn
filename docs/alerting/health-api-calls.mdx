...
The `selection criteria` are key/value pairs, in the format `key : value`, where value is a Netdata [simple pattern](/docs/miscellaneous/libnetdata/simple-patterns). This means that you can create very powerful selectors (you will rarely need more than one or two).
...
The test script under [tests/health_mgmtapi](/docs/miscellaneous/health-command-api-tester) contains a series of tests that you can either run or read through to understand the various calls and responses better.
