...
If you are [using our official native DEB/RPM packages](/docs/maintaining/update-netdata-agents#determine-which-installation-method-you-used), make sure the `netdata-plugin-perf` package is installed.
...
To enable it, use `edit-config` from the Netdata [config directory](/docs/configuring/configuration), which is typically at `/etc/netdata`, to edit the `netdata.conf` file.
...
Change the value of the `perf` setting to `yes` in the `[plugins]` section. Save the file and restart the Netdata Agent with `sudo systemctl restart netdata`, or the [appropriate method](/docs/maintaining/starting-and-stopping-netdata-agents) for your system.
