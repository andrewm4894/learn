...
sidebar_position: "618040"
slug: "/data-collection/apm/statsd"
---
...
- [K6 load testing tool](https://k6.io)
  ...
  - [Documentation](/docs/data-collection/synthetic-checks/k6-load-test)
...
- [Asterisk](https://www.asterisk.org/)
  ...
  - [Documentation](/docs/data-collection/telephony-servers/asterisk)
...
Private charts are controlled with `create private charts for metrics matching = *`. This setting accepts a space-separated list of [simple patterns](/docs/miscellaneous/libnetdata/simple-patterns). Netdata will create private charts for all metrics **by default**.
...
You can specify Netdata StatsD to have a different `memory mode` than the rest of the Netdata Agent. You can read more about `memory mode` in the [documentation](/docs/configuring/optimizing-metrics-database/database-modes-for-parent-child-setups).
...
-   `metrics` is a Netdata [simple pattern](/docs/miscellaneous/libnetdata/simple-patterns). This pattern should match all the possible StatsD metrics that will be participating in the application `myapp`.
...
`pattern` is a keyword. When set, `METRIC` is expected to be a Netdata [simple pattern](/docs/miscellaneous/libnetdata/simple-patterns) that will be used to match all the StatsD metrics to be added to the chart. So, `pattern` automatically matches any number of StatsD metrics, all of which will be added as separate chart dimensions.
...
- Create a configuration file for your app using [edit-config](/docs/configuring/configuration): `sudo ./edit-config
  statsd.d/myapp.conf`
...
- A node with the [Netdata](/docs/installing) installed.
...
Use [`edit-config`](/docs/configuring/configuration#use-edit-config-to-edit-configuration-files) 
...
- **pattern**: A keyword that tells the StatsD server the `METRIC` string is actually a 
  [simple pattern](/docs/miscellaneous/libnetdata/simple-patterns). 
  ...
  patterns in [dimension patterns](/docs/data-collection/apm/statsd#dimension-patterns).
...
To enable this StatsD configuration, [restart Netdata](/docs/maintaining/starting-and-stopping-netdata-agents).
...
To edit the section, please follow the Netdata [documentation](/docs/visualizations/legacy-agent-dashboard#customizing-the-local-dashboard).
...
- Make sure you follow the [contributing guide](/docs/contributing) 
