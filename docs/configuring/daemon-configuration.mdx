...
sidebar_label: "Daemon configuration"
learn_status: "Published"
learn_rel_path: "Configuring"
...
the [web server access lists](/docs/configuring/securing-netdata-agents/web-server#access-lists).

`netdata.conf` has sections stated with `[section]`. You will see the following sections:

1. `[global]` to [configure](#global-section-options) the [Netdata daemon](/docs/configuring/netdata-daemon).
...
5. `[environment variables]` to [configure](#environment-variables-section-options) the environment variables used
   Netdata.
6. `[sqlite]` to [configure](#sqlite-section-options) the [Netdata daemon](/docs/configuring/netdata-daemon) SQLite settings.
7. `[ml]` to configure settings for [machine learning](/docs/ml-and-troubleshooting/machine-learning-ml-powered-anomaly-detection).
8. `[health]` to [configure](#health-section-options) general settings for [health monitoring](/docs/alerting).
9. `[web]` to [configure the web server](/docs/configuring/securing-netdata-agents/web-server).
10. `[registry]` for the [Netdata registry](/docs/configuring/securing-netdata-agents/registry).
11. `[global statistics]` for the [Netdata registry](/docs/configuring/securing-netdata-agents/registry).
12. `[statsd]` for the general settings of the [stats.d.plugin](/docs/data-collection/apm/statsd).
13. `[plugins]` to [configure](#plugins-section-options) which [collectors](/docs/data-collection) to use and PATH
    settings.
...
After `netdata.conf` has been modified, Netdata needs to be [restarted](/docs/maintaining/starting-and-stopping-netdata-agents) for
changes to apply:
...
|:----------------------------------:|:-------------:|:---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|     process scheduling policy      |    `keep`     | See [Netdata process scheduling policy](/docs/configuring/netdata-daemon#netdata-process-scheduling-policy)                                                                                                   |
|             OOM score              |      `0`      |                                                                                                                                                                                                                                              |
| glibc malloc arena max for plugins |      `1`      | See [Virtual memory](/docs/configuring/netdata-daemon#virtual-memory).                                                                                                                                        |
| glibc malloc arena max for Netdata |      `1`      | See [Virtual memory](/docs/configuring/netdata-daemon#virtual-memory).                                                                                                                                        |
...
|                   retention                   |   `3600`   | Used with `mode = save/map/ram/alloc`, not the default `mode = dbengine`. This number reflects the number of entries the `netdata` daemon will by default keep in memory for each chart dimension. Check [Memory Requirements](/docs/configuring/optimizing-metrics-database/database-modes-for-parent-child-setups) for more information.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
|                 storage tiers                 |    `1`     | The number of storage tiers you want to have in your dbengine. Check the tiering mechanism in the [dbengine's reference](/docs/architecture/database-engine#tiering). You can have up to 5 tiers of data (including the _Tier 0_). This number ranges between 1 and 5.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
|                 update every                  |    `1`     | The frequency in seconds, for data collection. For more information see the [performance guide](/docs/configuring/how-to-optimize-the-netdata-agent-s-performance). These metrics stored as _Tier 0_ data. Explore the tiering mechanism in the [dbengine's reference](/docs/architecture/database-engine#tiering).                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
|          memory deduplication (ksm)           |   `yes`    | When set to `yes`, Netdata will offer its in-memory round robin database and the dbengine page cache to kernel same page merging (KSM) for deduplication. For more information check [Memory Deduplication - Kernel Same Page Merging - KSM](/docs/configuring/optimizing-metrics-database/database-modes-for-parent-child-setups#ksm)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
|      cleanup obsolete charts after secs       |   `3600`   | See [monitoring ephemeral containers](/docs/data-collection/containers-and-vms/cgroups#monitoring-ephemeral-containers), also sets the timeout for cleaning up obsolete dimensions                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
|         delete obsolete charts files          |   `yes`    | See [monitoring ephemeral containers](/docs/data-collection/containers-and-vms/cgroups#monitoring-ephemeral-containers), also affects the deletion of files for obsolete dimensions                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
|         log         |                         `/var/log/netdata`                         | The directory in which the [log files](/docs/configuring/netdata-daemon#log-files) are kept.                                                        |
...
|      registry       |              `/opt/netdata/var/lib/netdata/registry`               | Contains the [registry](/docs/configuring/securing-netdata-agents/registry) database and GUID that uniquely identifies each Netdata Agent                           |

### [logs] section options

|              setting               |            default            | info                                                                                                                                                                                                                                                                                                                             |
|:----------------------------------:|:-----------------------------:|:---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|            debug flags             |     `0x0000000000000000`      | Bitmap of debug options to enable. For more information check [Tracing Options](/docs/configuring/netdata-daemon#debugging).                                                                                                                                                                      |
|               debug                | `/var/log/netdata/debug.log`  | The filename to save debug information. This file will not be created if debugging is not enabled. You can also set it to `syslog` to send the debug messages to syslog, or `none` to disable this log. For more information check [Tracing Options](/docs/configuring/netdata-daemon#debugging). |
...
monitoring](/docs/alerting).

[Alert notifications](/docs/alerting/notifications/netdata-agent-notifications) are configured in `health_alarm_notify.conf`.
...
|                 enabled alarms                 |                        *                         | Defines which alerts to load from both user and stock directories. This is a [simple pattern](/docs/miscellaneous/libnetdata/simple-patterns) list of alert or template names. Can be used to disable specific alerts. For example, `enabled alarms =  !oom_kill *` will load all alerts except `oom_kill`. |

### [web] section options

Refer to the [web server documentation](/docs/configuring/securing-netdata-agents/web-server)
...
the [registry documentation](/docs/configuring/securing-netdata-agents/registry).
...
Most internal plugins will provide additional options. Check [Internal Plugins](/docs/data-collection) for more
information.
...
|     setting     |                   default                    | info                                                                                                                                                                                                |
|:---------------:|:--------------------------------------------:|:----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|  update every   | the value of `[global].update every` setting | The frequency in seconds the plugin should collect values. For more information check the [performance guide](/docs/configuring/how-to-optimize-the-netdata-agent-s-performance). |
